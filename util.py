...
def get_raising_discussion_score(masuda_id, bookmark_count):
    if bookmark_count == 0:
        return 0
    url = "https://b.hatena.ne.jp/entry/s/anond.hatelabo.jp/{masuda_id}".format(
        masuda_id=masuda_id)
    res = requests.get(url)
    soup = BeautifulSoup(res.text, 'html.parser')
    if soup.find('span', 'entry-info-comment'):
        comment_count = int(
            soup.find('span', 'entry-info-comment').find('span').get_text())
        if comment_count != 0:
...
def is_include_kata(text):
    return 1 if '方' in text else 0
